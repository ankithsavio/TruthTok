...
def download_videos_from_search(search_query, max_duration=300, max_videos=10, output_dir="video_model/data"):
    """
    ...
    ydl_opts = {
        ...
        "outtmpl": os.path.join(output_dir + "/videos", "%(title)s.%(ext)s"),  # Output filename template
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        for entry in search_results["entries"]:
            ...
            if video_duration and video_duration <= max_duration:
                ...
                output_path = os.path.join(output_dir + "/videos", f"{sanitized_title}.mp4")
                ydl_opts["outtmpl"] = output_path  # Update output path for the sanitized filename
    ...
    metadata_path = os.path.join(metadata_dir, "video.json")
